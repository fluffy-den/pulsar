...
namespace pul
{
	...
	TEST_CASE("MathVec4Benchmarks")
	{
		...
		(Catch::Benchmark::Chronometer __m)
		{
			...
			auto buf = std::make_unique<f32vec4u_t[]>(n);
		...
		(Catch::Benchmark::Chronometer __m)
		{
			...
			std::uniform_real_distribution<float32_t> r(
					std::numeric_limits<float32_t>::min(),
					std::numeric_limits<float32_t>::max());
			auto buf = std::make_unique<f32vec4_t[]>(n);
			for (auto b = &buf[0], e = &buf[n - 1]; b != e; ++b)
			{
				*b = { r(gen), r(gen), r(gen), r(gen) };
			}
		...
		(Catch::Benchmark::Chronometer __m)
		{
			...
			std::uniform_real_distribution<float32_t> r(
					std::numeric_limits<float32_t>::min(),
					std::numeric_limits<float32_t>::max());
			auto rbuf = std::make_unique<f32vec4u_t[]>(n);
			auto cbuf = std::make_unique<f32vec4u_t[]>(n);
		...
		(Catch::Benchmark::Chronometer __m)
		{
			...
			std::uniform_real_distribution<float32_t> r(
					std::numeric_limits<float32_t>::min(),
					std::numeric_limits<float32_t>::max());
			auto rbuf = std::make_unique<f32vec4_t[]>(n);
			auto cbuf = std::make_unique<f32vec4_t[]>(n);
			for (auto b = &rbuf[0], e = &rbuf[n - 1]; b != e; ++b)
			{
				*b = { r(gen), r(gen), r(gen), r(gen) };
			}
