...
namespace pul
{
	...
	/// SEQUENCE: Forward -> Swap
	template <typename _Ty,
						typename _MagnifierL, typename _AllocatorL,
						typename _MagnifierR, typename _AllocatorR>
	pf_decl_constexpr void
	swap(
		forward_sequence<_Ty, _MagnifierL, _AllocatorL> &__l,
		forward_sequence<_Ty, _MagnifierR, _AllocatorR> &__r) pf_attr_noexcept
	{
		__l.swap(__r);
	}

	/// SEQUENCE: Forward -> Begin
	template <typename _Ty, typename _Magnifier, typename _Allocator>
	pf_hint_nodiscard pf_decl_constexpr auto
	begin(
		forward_sequence<_Ty, _Magnifier, _Allocator> &__s) pf_attr_noexcept
	{
		return __s.begin();
	}
	template <typename _Ty, typename _Magnifier, typename _Allocator>
	pf_hint_nodiscard pf_decl_constexpr auto
	begin(
		forward_sequence<_Ty, _Magnifier, _Allocator> const &__s) pf_attr_noexcept
	{
		return __s.begin();
	}

	/// SEQUENCE: Forward -> End
	template <typename _Ty, typename _Magnifier, typename _Allocator>
	pf_hint_nodiscard pf_decl_constexpr auto
	end(
		forward_sequence<_Ty, _Magnifier, _Allocator> &__s) pf_attr_noexcept
	...
	/// SEQUENCE: Forward -> Reverse Begin
	template <typename _Ty, typename _Magnifier, typename _Allocator>
	pf_hint_nodiscard pf_decl_constexpr auto
	rbegin(
		forward_sequence<_Ty, _Magnifier, _Allocator> &__s) pf_attr_noexcept
	...
	/// SEQUENCE: Forward -> Const Reverse Begin
	template <typename _Ty, typename _Magnifier, typename _Allocator>
	pf_hint_nodiscard pf_decl_constexpr auto
	crbegin(
		forward_sequence<_Ty, _Magnifier, _Allocator> &__s) pf_attr_noexcept
	...
	/// SEQUENCE: Forward -> Const Reverse End
	template <typename _Ty, typename _Magnifier, typename _Allocator>
	pf_hint_nodiscard pf_decl_constexpr auto
	crend(
		forward_sequence<_Ty, _Magnifier, _Allocator> const &__s) pf_attr_noexcept
	...
	private:
		_Ty *store_;
		size_t beg_;// NOTE: As efficient as forward_sequence if using magnifier_default.
		size_t end_;
		size_t memCount_;
		align_val_t memAlign_;
		_Magnifier magnifier_;
