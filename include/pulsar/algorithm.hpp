...
namespace pul
{
	...
	template<
	 ...
	 typename _IteratorOut>
	pf_decl_constexpr void
	copy_reverse(_IteratorIn __beg, _IteratorIn __end, _IteratorOut __dst)
		requires(is_iterator_v<_IteratorIn> && is_iterator_v<_IteratorOut>)
	{
		--__beg;
		--__end;
	...
	copy(
	 ...
	 _IteratorIn __end,
	 _IteratorOut __dst) pf_attr_noexcept
		requires(is_sequence_iterator_v<_IteratorIn> && is_sequence_iterator_v<_IteratorOut>)
	{
	...
	search(
	 ...
	 _IteratorInB __oe) pf_attr_noexcept
		requires(is_iterator_v<_IteratorInA> && is_iterator_v<_IteratorInB>)
	{
		while(1)
		{
	...
	equal(
	 ...
	 _IteratorInB __bb) pf_attr_noexcept
		requires(is_iterator_v<_IteratorInA> && is_iterator_v<_IteratorInB>)
	{
		for(; __ba != __ea; ++__ba, ++__bb)
			if(*__ba != *__bb) return false;
		return true;
	}
	template<
	 typename _IteratorInA,
	...
	equal(
	 ...
	 _IteratorInA __ea,
	 _Uy const &__val)
		requires(is_iterator_v<_IteratorInA> && !is_iterator_v<_Uy>)	// TODO: is_equality_comparable_v<_IteratorInA, _Uy>
	{
		for(; __ba != __ea; ++__ba)
			if(*__ba != __val) return false;
		return true;
	}
	template<
	 typename _IteratorInA,
	...
	template<
	 typename _IteratorIn,
	 typename _Predicate>
	pf_hint_nodiscard pf_decl_constexpr _IteratorIn
	find(
	 _IteratorIn __beg,
	 _IteratorIn __end,
