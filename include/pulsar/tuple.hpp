...
namespace pul
{
	...
	struct __tuple_decrement_if_greater_equal_to_index<__tuple_base<_Ts...>, _Index>
	{
		using tuple_type = __tuple_base<
		 __tuple_index<typename _Ts::type, ((_Ts::index > _Index) ? _Ts::index - 1 : _Ts::index)>...>;
	};
	...
	private:
		...
		private:
			using next_tuple_type = std::conditional_t<
			 _Comparator<typename tuple_element<_TupleLoop, _I>::type, typename tuple_element<_TupleLoop, _J>::type>::value,
...
#pragma GCC diagnostic ignored "-Wpedantic"
	...
	union pair
	{
		...
		pf_decl_constexpr
		pair(pair<_TyA, _TyB> &&) pf_attr_noexcept = default;
...
#pragma GCC diagnostic ignored "-Wpedantic"
	...
	pf_hint_nodiscard pf_decl_constexpr nutag<_Ty, _Hash>
	nuval(
	 _Ty &&__val)
	{
	...
	template<size_t _Hash, typename _Nuple>
	pf_hint_nodiscard pf_decl_constexpr size_t
	n_offsetof(_Nuple const &__nuple) pf_attr_noexcept
	...
	template<size_t _Hash, typename _Nuple>
	pf_hint_nodiscard pf_decl_constexpr size_t
	n_sizeof(_Nuple const &__nuple) pf_attr_noexcept
		requires(is_nuple_v<_Nuple>)
	{
