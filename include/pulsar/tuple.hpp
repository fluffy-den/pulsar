...
#include "pulsar/tuple/nuple.hpp"
#include "pulsar/tuple/pair.hpp"
...
namespace pul
{
	...
	{
		return std::move(__param_pack_extract_at_index<_Index - 1>(std::forward<_InTs>(__args)...));
	}
	...
	public:
		...
		/// Operator=
		pf_decl_inline pf_decl_constexpr __tuple_base<_Ty, _Ts...> &
	...
	struct __tuple_base<_Ty>
	{
		...
		pf_decl_inline pf_decl_constexpr __tuple_base(_InTy &&__arg)
			: elem_{std::forward<_InTy>(__arg)}
		{}
		pf_decl_inline pf_decl_explicit pf_decl_constexpr
		__tuple_base(__tuple_base<_Ty> const &) = default;
		pf_decl_inline pf_decl_explicit pf_decl_constexpr
		__tuple_base(__tuple_base<_Ty> &&)			= default;

		/// Operator=
		pf_decl_inline pf_decl_constexpr __tuple_base<_Ty> &
	...
	struct __tuple_decrement_if_greater_equal_to_index<
		...
		_Index>
	{
		using tuple_type = (__tuple_base < __tuple_index < typename _Ts::type),
	...
	struct tuple : public __tuple_select_base<_Ts...>
	{
		...
		/// Operator=
		pf_decl_inline pf_decl_constexpr tuple<_Ts...> &
	...
	{
		return pul::addressof(&i_get<_Index>(__tuple)) - pul::addressof(&__tuple);
	}
...
#define $(name) xx::hash(name)
	...
	struct nuple : public __nuple_select_base<_Ts ...>
	{
		...
		/// Operator=
		pf_decl_inline pf_decl_constexpr nuple<_Ts ...> &
